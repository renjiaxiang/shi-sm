...
//define('IN_jylbsh_ADMIN', true);

require_once '../framework/bootstrap.inc.php';
//require_once IA_ROOT . '/web/common/bootstrap.sys.inc.php';

define('jylbsh_ROOT', dirname(dirname(__FILE__)));
define('IS_OPERATOR', true);
require_once jylbsh_ROOT . '/addons/jy_lbsh/admin/common/attachurl.php';
//echo $_W['attachurl'];die;
require_once jylbsh_ROOT . '/addons/jy_lbsh/admin/common/bootstrap.sys.inc.php';
require_once jylbsh_ROOT . '/addons/jy_lbsh/admin/common/template.func.php';
require_once jylbsh_ROOT . '/addons/jy_lbsh/admin/common/common.func.php';
// require_once jylbsh_ROOT . '/addons/jy_lbsh/admin/common/file.func.php';
// global $_GPC, $_W;
...
if (($_W['setting']['copyright']['status'] == 1) && empty($_W['isfounder']) && $controller != 'cloud' && $controller != 'utility' && $controller != 'account') {
	...
	if ($controller == 'user' && $action == 'login') {
		if (checksubmit()) {
			require_once _forward($controller, $action);
		}
		template('user/login');
		exit;
	}
...
function _forward($c, $a) {
	$file = jylbsh_ROOT . '/addons/jy_lbsh/admin/source/' . $c . '/' . $a . '.ctrl.php';
	// var_dump($file);die;
	return $file;
}
